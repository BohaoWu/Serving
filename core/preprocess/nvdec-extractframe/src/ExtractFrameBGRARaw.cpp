        ...
        for (auto i = 0; i < frame_returned; ++i) {
            ...
            if (nvdec.GetBitDepth() == 8) {
                if (nvdec.GetOutputFormat() == cudaVideoSurfaceFormat_YUV444)
                    YUV444ToColor32<BGRA32>((uint8_t *) pp_frame[i], nvdec.GetWidth(),
                                            (uint8_t *) p_tmp_image, 4 * nvdec.GetWidth(),
                                            nvdec.GetWidth(), nvdec.GetHeight());
                else {
                    Nv12ToColor32<BGRA32>((uint8_t *) pp_frame[i], nvdec.GetWidth(),
                                          (uint8_t *) p_tmp_image, 4 * nvdec.GetWidth(),
                                          nvdec.GetWidth(), nvdec.GetHeight());
                }
                GetImage(p_tmp_image, fm_tmp.get_frame(), nvdec.GetWidth(), 4 * nvdec.GetHeight());
            } else {
                if (nvdec.GetOutputFormat() == cudaVideoSurfaceFormat_YUV444_16Bit)
                    YUV444P16ToColor32<BGRA32>((uint8_t *) pp_frame[i], 2 * nvdec.GetWidth(),
                                               (uint8_t *) p_tmp_image, 4 * nvdec.GetWidth(),
                                               nvdec.GetWidth(), nvdec.GetHeight());
                else
                    P016ToColor32<BGRA32>((uint8_t *) pp_frame[i], nvdec.GetWidth(),
                                          (uint8_t *) p_tmp_image, 4 * nvdec.GetWidth(),
                                          nvdec.GetWidth(), nvdec.GetHeight());
                GetImage(p_tmp_image, fm_tmp.get_frame(), nvdec.GetWidth(), 4 * nvdec.GetHeight());
            }
            /*GetImage((CUdeviceptr) pp_frame[i], reinterpret_cast<uint8_t *>(fm_tmp.p_frame.get()),
                     nvdec.GetWidth(),
                     nvdec.GetHeight() + (nvdec.GetChromaHeight() * nvdec.GetNumChromaPlanes()));*/
        }
    } while (n_video_bytes);
