...
#ifndef HWEXTRACT_FRAME_BASE
#define HWEXTRACT_FRAME_BASE
namespace baidu {
namespace xvision {
...
public:
    ...
    FrameResult deepcopy() const {
        ...
        std::copy(this -> get_frame(), this -> get_frame() + this -> get_frame_buff_len(), frame_tmp);
    ...
    */
    ...
    void set_thick(const size_t thick) {
        _thick = thick;
    }
    ...
    size_t get_frame_buff_len() const{
        return _rows * _cols * _thick;
    }
    virtual ~FrameResult() {
    }
...
class ExtractFrameBase {
     ...
     **/
public:
    ExtractFrameBase(int gpu_index) {
    ...
    virtual int init();
    virtual IMGDataList extract_frame(const std::string& file_path, int n = 1) = 0;
     ...
     * @param, mode, 0, use time stamp to select frame, 1, use framerate to select frame
     **/
    static bool select_frame(const double frame_rate, const int64_t pre_frame_time, \
        const int64_t cur_frame_time, const size_t frame_index, const double fps, const int mode);
